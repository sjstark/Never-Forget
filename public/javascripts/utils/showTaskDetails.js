...
const createInputField = (editContainer) => {
  ...
  editContainer.onclick = null;

  inputField.addEventListener('focusout', async (e) => {

    inputField.onkeyup = null;

    let isValidOrErrors = validateCreateInputField(inputField.id, inputField.value,10)
...
const validateCreateInputField = (inputFieldId, inputFieldValue) => {
  ...
  if (inputFieldId === 'dueDate') {
    ...
    if (inputFieldValue === 'none' || inputFieldValue === '' || regexDate.test(inputFieldValue)) {
      // continue
    } else {
  ...
  if (inputFieldId === 'estimate') {
    let parsedNum = parseInt(inputFieldValue, 10)

    if ((Number.isInteger(parsedNum) && parsedNum >= 0) || inputFieldValue === 'none' || inputFieldValue === '' || inputFieldValue === '0') {
      // continue
    } else {
